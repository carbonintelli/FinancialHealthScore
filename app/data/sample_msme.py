"""Sample MSME data for demonstrations and testing."""

from datetime import date

from app.models.schemas import (
    AccountingSnapshot,
    AssessmentRequest,
    AudienceRole,
    CreditBureauProfile,
    FinancialDataInput,
    FounderProfile,
    GovernanceDiversityProfile,
    ESGDisclosureProfile,
    SupplyChainProfile,
    InsuranceProfile,
    GeographicProfile,
    DocumentUpload,
    GovernmentComplianceProfile,
    GovernmentPolicyEnrollment,
    LegalCaseRecord,
    LegalComplianceProfile,
    LoanRepaymentRecord,
    MarketSentiment,
    MonthlyCashFlow,
    MSMEProfile,
    OperationalCertification,
    OperationalCertificationProfile,
    PastDebtRecord,
    PaymentRecord,
    ProductLine,
    ProductMarketProfile,
    TaxComplianceProfile,
    UtilityBill,
)


def build_demo_request(audience: AudienceRole = AudienceRole.CREDIT_TEAM) -> AssessmentRequest:
    """Build a realistic demo MSME assessment request for IDBI Innovate 2026."""
    return AssessmentRequest(
        financial_data=FinancialDataInput(
            profile=MSMEProfile(
                msme_id="msme-demo-001",
                business_name="Shree Ganesh Auto Components Pvt Ltd",
                udyam_number="UDYAM-MH-12-0012345",
                gstin="27AABCS1234F1Z5",
                pan="AABCS1234F",
                state="maharashtra",
                pincode="411045",
                sector="auto_components",
                employee_count=85,
                years_in_operation=12,
                annual_turnover_inr=48_000_000,
            ),
            accounting=AccountingSnapshot(
                revenue_inr=48_000_000,
                cost_of_goods_inr=31_200_000,
                operating_expenses_inr=9_600_000,
                current_assets_inr=14_500_000,
                current_liabilities_inr=8_200_000,
                total_debt_inr=12_000_000,
                equity_inr=18_500_000,
                net_profit_inr=4_800_000,
                period_end=date(2026, 3, 31),
            ),
            cash_flows=[
                MonthlyCashFlow(month="2026-01", inflows=4_100_000, outflows=3_750_000),
                MonthlyCashFlow(month="2026-02", inflows=3_850_000, outflows=3_620_000),
                MonthlyCashFlow(month="2026-03", inflows=4_450_000, outflows=3_890_000),
                MonthlyCashFlow(month="2026-04", inflows=4_200_000, outflows=3_800_000),
                MonthlyCashFlow(month="2026-05", inflows=3_950_000, outflows=3_700_000),
                MonthlyCashFlow(month="2026-06", inflows=4_300_000, outflows=3_850_000),
            ],
            utility_bills=[
                UtilityBill(month="2026-01", electricity_kwh=18500, electricity_cost_inr=148_000, fuel_litres=2200, fuel_cost_inr=198_000),
                UtilityBill(month="2026-02", electricity_kwh=17200, electricity_cost_inr=138_000, fuel_litres=2050, fuel_cost_inr=185_000),
                UtilityBill(month="2026-03", electricity_kwh=19100, electricity_cost_inr=153_000, fuel_litres=2350, fuel_cost_inr=212_000),
            ],
            payment_records=[
                PaymentRecord(counterparty="Tata Steel Ltd", amount_inr=850_000, due_date=date(2026, 4, 15), paid_date=date(2026, 4, 14), status="on_time"),
                PaymentRecord(counterparty="Mahindra Logistics", amount_inr=320_000, due_date=date(2026, 4, 20), paid_date=date(2026, 4, 22), status="late"),
                PaymentRecord(counterparty="Bharat Forge", amount_inr=1_200_000, due_date=date(2026, 5, 1), paid_date=date(2026, 4, 28), status="on_time"),
                PaymentRecord(counterparty="Local Supplier Co", amount_inr=95_000, due_date=date(2026, 5, 10), paid_date=date(2026, 5, 10), status="on_time"),
                PaymentRecord(counterparty="MSEB Electricity", amount_inr=148_000, due_date=date(2026, 4, 25), paid_date=date(2026, 4, 25), status="on_time"),
            ],
            bank_statement_summary={
                "avg_monthly_balance_inr": 3_200_000,
                "min_monthly_balance_inr": 1_100_000,
                "cheque_bounce_count_12m": 1,
            },
            founder=FounderProfile(
                name="Rajesh Patil",
                is_female=False,
                years_industry_experience=18,
                years_entrepreneurship=12,
                education_level="graduate",
                prior_business_exits=0,
                cibil_score=782,
                prior_defaults=0,
                management_team_size=4,
                succession_plan_documented=False,
                industry_certifications=["ISO 9001 Lead Auditor", "Lean Six Sigma Green Belt"],
                linkedin_presence_score=72,
            ),
            market_sentiment=MarketSentiment(
                overall_sentiment_score=74,
                customer_nps=52,
                google_rating=4.2,
                google_review_count=38,
                media_mentions_12m=6,
                positive_media_pct=83,
                customer_retention_rate_pct=86,
                supplier_trust_score=78,
                litigation_count_3y=0,
                gst_compliance_rating="good",
            ),
            product_market=ProductMarketProfile(
                products=[
                    ProductLine(name="Precision Forged Crankshafts", category="auto_components", revenue_share_pct=42, hsn_code="8483", is_export_oriented=True),
                    ProductLine(name="Transmission Gears", category="auto_components", revenue_share_pct=35, hsn_code="8483"),
                    ProductLine(name="EV Motor Housings", category="auto_components", revenue_share_pct=15, hsn_code="8501", is_export_oriented=False),
                    ProductLine(name="General Machined Parts", category="auto_components", revenue_share_pct=8, hsn_code="8483"),
                ],
                primary_market="both",
                export_revenue_pct=22,
                market_demand_outlook="moderate_growth",
                sector_growth_rate_pct=9.5,
                capacity_utilisation_pct=78,
                order_book_months=4.2,
                import_substitution_potential=True,
                ev_supply_chain_exposure=True,
            ),
            government_policy=GovernmentPolicyEnrollment(
                enrolled_scheme_codes=["CGTMSE", "CLCSS", "SAMADHAN"],
                pending_applications=["PLI_AUTO"],
                zed_certification_level="silver",
                iso_certifications=["ISO 9001:2015", "IATF 16949"],
                gst_filing_compliance_pct=96,
                delayed_payment_complaints_filed=1,
                received_govt_subsidy_inr=1_850_000,
            ),
            credit_bureau=CreditBureauProfile(
                crisil_rating="BBB+",
                crisil_outlook="stable",
                crisil_rating_date=date(2026, 2, 15),
                rating_agency="CRISIL",
                commercial_credit_score=3,
                emi_on_time_pct_12m=94.5,
                restructured_loans_count=0,
                written_off_loans_count=0,
                npa_incidents_5y=0,
                debt_service_coverage_ratio=1.62,
                past_debts=[
                    PastDebtRecord(
                        lender_name="IDBI Bank",
                        loan_type="term_loan",
                        principal_inr=8_000_000,
                        outstanding_inr=4_200_000,
                        interest_rate_pct=10.25,
                        tenure_months=84,
                        start_date=date(2021, 6, 1),
                        status="active",
                        repayment_completed_pct=47.5,
                        emi_amount_inr=128_500,
                        collateral_type="hypothecation",
                    ),
                    PastDebtRecord(
                        lender_name="HDFC Bank",
                        loan_type="cc_od",
                        principal_inr=3_000_000,
                        outstanding_inr=1_100_000,
                        interest_rate_pct=11.5,
                        start_date=date(2019, 3, 15),
                        closed_date=date(2024, 3, 14),
                        status="closed",
                        repayment_completed_pct=100,
                        collateral_type="none",
                    ),
                    PastDebtRecord(
                        lender_name="SIDBI",
                        loan_type="equipment_finance",
                        principal_inr=2_500_000,
                        outstanding_inr=0,
                        interest_rate_pct=9.75,
                        tenure_months=60,
                        start_date=date(2018, 1, 10),
                        closed_date=date(2023, 1, 9),
                        status="closed",
                        repayment_completed_pct=100,
                        collateral_type="equipment",
                    ),
                ],
                repayment_history=[
                    LoanRepaymentRecord(month="2026-01", lender_name="IDBI Bank", emi_amount_inr=128_500, due_date=date(2026, 1, 5), paid_date=date(2026, 1, 5), status="on_time", outstanding_after_inr=4_350_000),
                    LoanRepaymentRecord(month="2026-02", lender_name="IDBI Bank", emi_amount_inr=128_500, due_date=date(2026, 2, 5), paid_date=date(2026, 2, 6), status="late", outstanding_after_inr=4_280_000),
                    LoanRepaymentRecord(month="2026-03", lender_name="IDBI Bank", emi_amount_inr=128_500, due_date=date(2026, 3, 5), paid_date=date(2026, 3, 5), status="on_time", outstanding_after_inr=4_200_000),
                    LoanRepaymentRecord(month="2026-04", lender_name="IDBI Bank", emi_amount_inr=128_500, due_date=date(2026, 4, 5), paid_date=date(2026, 4, 5), status="on_time", outstanding_after_inr=4_120_000),
                    LoanRepaymentRecord(month="2026-05", lender_name="IDBI Bank", emi_amount_inr=128_500, due_date=date(2026, 5, 5), paid_date=date(2026, 5, 5), status="on_time", outstanding_after_inr=4_040_000),
                    LoanRepaymentRecord(month="2026-06", lender_name="IDBI Bank", emi_amount_inr=128_500, due_date=date(2026, 6, 5), paid_date=date(2026, 6, 5), status="on_time", outstanding_after_inr=3_960_000),
                ],
            ),
            legal_compliance=LegalComplianceProfile(
                company_lawsuits=[
                    LegalCaseRecord(
                        case_type="civil",
                        party="Former Employee",
                        filed_against="company",
                        status="resolved_favorable",
                        amount_at_stake_inr=450_000,
                        filing_date=date(2023, 8, 10),
                        description="Wrongful termination claim — dismissed in company favor",
                    ),
                ],
                founder_lawsuits=[],
                pending_cases_company=0,
                pending_cases_founders=0,
                resolved_favorable_pct=100,
                regulatory_penalties_3y=0,
                criminal_cases_pending=0,
                arbitration_cases_pending=0,
            ),
            tax_compliance=TaxComplianceProfile(
                itr_filed_on_time_3y=3,
                income_tax_paid_inr_12m=1_120_000,
                advance_tax_compliance_pct=100,
                tds_compliance_pct=97,
                gst_filing_compliance_pct=96,
                tax_demand_outstanding_inr=0,
                tax_litigation_pending=False,
                last_itr_acknowledgement="ITR-6/2025-26/1234567890",
                tax_clearance_certificate=False,
            ),
            operational_certifications=OperationalCertificationProfile(
                certifications=[
                    OperationalCertification(name="ISO 9001:2015", certifying_body="Bureau Veritas", valid_until=date(2027, 6, 30), scope="Quality Management"),
                    OperationalCertification(name="IATF 16949:2016", certifying_body="TUV SUD", valid_until=date(2027, 3, 15), scope="Automotive Quality"),
                    OperationalCertification(name="ISO 14001:2015", certifying_body="Bureau Veritas", valid_until=date(2026, 12, 31), scope="Environmental Management"),
                ],
                iso_certifications=["ISO 9001:2015", "IATF 16949", "ISO 14001:2015"],
                last_quality_audit_date=date(2026, 1, 20),
                quality_audit_passed=True,
                certification_coverage_pct=88,
            ),
            government_compliance=GovernmentComplianceProfile(
                labour_law_compliance_pct=94,
                pf_esi_compliance_pct=98,
                environmental_clearance_valid=True,
                pollution_control_board_consent=True,
                factory_act_registered=True,
                shops_establishment_valid=True,
                statutory_audit_completed=True,
                mca_annual_filings_current=True,
                msme_samadhaan_registered=True,
                fire_safety_noc_valid=True,
                overall_compliance_score=92,
            ),
            governance_diversity=GovernanceDiversityProfile(
                female_founders_count=1,
                female_directors_count=2,
                total_directors=5,
                women_led_enterprise=False,
                women_entrepreneur_scheme_enrolled=False,
                board_independence_pct=40,
            ),
            esg_disclosure=ESGDisclosureProfile(
                brsr_lite_ready=False,
                ghg_inventory_completed=True,
                esg_report_published=False,
                disclosure_score=62,
                supplier_esg_program=True,
                water_stress_disclosure=True,
                waste_management_program=True,
            ),
            supply_chain=SupplyChainProfile(
                key_supplier_count=24,
                key_customer_count=18,
                single_source_dependency_pct=22,
                inventory_days=42,
                alternate_suppliers_identified_pct=75,
                stress_scenario_survival_months=4.5,
                import_dependency_pct=15,
            ),
            insurance=InsuranceProfile(
                property_insurance=True,
                machinery_breakdown_cover=True,
                business_interruption_cover=True,
                liability_insurance=True,
                key_person_insurance=False,
                coverage_adequacy_pct=82,
                total_coverage_inr=55_000_000,
                claims_history_clean=True,
            ),
            geographic=GeographicProfile(
                state="maharashtra",
                district="pune",
                tier="tier2",
                flood_risk_zone="low",
                industrial_cluster_presence=True,
            ),
            documents=[
                DocumentUpload(document_type="itr", file_name="ITR_FY2025-26.pdf"),
                DocumentUpload(document_type="audit_report", file_name="Statutory_Audit_2025.pdf"),
                DocumentUpload(document_type="bank_statement", file_name="Bank_Summary_12M.pdf"),
            ],
        ),
        include_carbon_intelligence=True,
        audience=audience,
    )


def build_minimal_request(
    msme_id: str,
    business_name: str,
    annual_revenue: float,
    audience: AudienceRole = AudienceRole.PORTFOLIO_ANALYST,
) -> AssessmentRequest:
    """Build minimal financial input when only Carbon Intelligence data is available."""
    estimated_cogs = annual_revenue * 0.65
    estimated_opex = annual_revenue * 0.20
    return AssessmentRequest(
        financial_data=FinancialDataInput(
            profile=MSMEProfile(
                msme_id=msme_id,
                business_name=business_name,
                sector="manufacturing",
            ),
            accounting=AccountingSnapshot(
                revenue_inr=annual_revenue,
                cost_of_goods_inr=estimated_cogs,
                operating_expenses_inr=estimated_opex,
                current_assets_inr=annual_revenue * 0.30,
                current_liabilities_inr=annual_revenue * 0.18,
                total_debt_inr=annual_revenue * 0.25,
                equity_inr=annual_revenue * 0.35,
            ),
        ),
        include_carbon_intelligence=True,
        audience=audience,
    )
